  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-800/50 p-6 rounded-lg border border-gray-700 shadow-lg mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          <label htmlFor="referencia" className="block text-sm font-medium text-gray-300 mb-1">Referência</label>
          <div className="flex">
            <input
              type="text"
              id="referencia"
            ...
            <button
              ...
              className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 px-4 rounded-r-md flex items-center justify-center gap-2 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors flex-shrink-0"
            >
              {isFetchingVerse ? (
                <>
                ...
                <>
                  ...
                  <span className="text-sm whitespace-nowrap">Buscar Texto</span>
                </>
              )}
            </button>
          </div>
          <p className="text-xs text-cyan-400/80 mt-2 flex items-start gap-1">
            <span className="mt-0.5">💡</span>
            <span>Digite a referência e clique em "Buscar Texto" para preencher a descrição automaticamente.</span>
          </p>
        </div>
        <div>
      ...
      <div className="mb-4">
        <label htmlFor="descricao" className="block text-sm font-medium text-gray-300 mb-1">
          Descrição (Preenchimento Automático via IA)
        </label>
        <textarea
          id="descricao"
          value={descricao}
          ...
          rows={4}
          placeholder="O texto do versículo aparecerá aqui automaticamente após clicar no botão 'Buscar Texto'."
          className="w-full bg-gray-900 border border-gray-600 rounded-md px-3 py-2 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition placeholder-gray-500 text-white"
        />
      </div>
